package dev.sukhrob.familybudget.view.details
...
import dev.sukhrob.familybudget.databinding.FragmentTransactionDetailsBinding
import dev.sukhrob.familybudget.data.model.Transaction
import dev.sukhrob.familybudget.utils.saveBitmap
...
class TransactionDetailsFragment : BaseFragment<FragmentTransactionDetailsBinding, TransactionViewModel>() {
    ...
    private fun showErrorDialog() =
        findNavController().navigate(
            TransactionDetailsFragmentDirections.actionTransactionDetailsFragmentToErrorDialog(
                "Rasm ulashilmadi!",
                "Tranzaksiyani Rasm sifatida almashish uchun 'saqlash' ruxsatini yoqishingiz kerak"
            )
        )
    ...
    private fun onDetailsLoaded(transaction: Transaction) = with(binding.transactionDetails) {
        ...
        amount.text = uzbekSum(transaction.amount).cleanTextContent
        //type.text = transaction.transactionType
        type.text = if (transaction.transactionType == "Expense") {
            "Xarajat"
        } else {
            "Daromad"
        }
